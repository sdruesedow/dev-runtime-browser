...
function editPrivacy(j, status){
  ...
  window.runtime.runtime.editContact(obj[j]._guid, obj[j]._firstName, obj[j]._lastName, obj[j]._guid, status);
  window.addEventListener("message", editContact, false);
}


function editContact(event){
  if(event.data.to == 'runtime:editContact'){
    let success = event.data.body.success;
    if(success){
      let result = event.data.body.contact;
      console.info(result);
      getAllContacts();
    }
  }
